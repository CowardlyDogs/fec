...
let ReviewModule = ({productId, theme}) => {
  ...
  let toggleNewReview = () => { setNewReviewVisible(!newReviewVisible); };
  useEffect(() => { Helpers.getReviews({productId, page}, (err, data) => { if (err) { console.error('Unable to fetch Reviews'); } else { setReviews(data.results); } }); Helpers.postReview(review, (err) => { if (err) { console.error('Unable to Post'); } }); }, [page, update, product_id]);
  let submitNewReview = (review) => {};
  let helpful = (review_id) => { Helpers.rateHelpful(review_id, (err) => { if (err) { console.error('Unable to Rate Helpful'); } else { setUpdate(update + 1); } }); };
  let report = (review_id) => { Helpers.rateReport(review_id,  (err) => { if (err) { console.error('Unable to Report'); } else { setUpdate(update + 1); } }); };
  let turnPage = (option) => { if (option === 'inc') { setPage(page + 1); } else if (option === 'dec') { setPage(page - 1); } };
  return (
